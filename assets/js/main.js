/*!
 * POGO SARABURI - MAIN JS v4.0.1
 * Fix: heroL1 white-space nowrap
 */
(function(){
'use strict';
...
function runEntrance(){
  ...
  // ── splitChars: เพิ่ม white-space:nowrap เพื่อไม่ให้ตัวอักษรขึ้นบรรทัดใหม่ตอน split ──
  function splitChars(el){
    var txt=el.textContent;
    el.innerHTML='';
    el.style.perspective='1200px';
    el.style.whiteSpace='nowrap'; // FIX: prevent wrapping
...
function initWordSwap(){
  ...
  setInterval(function(){
    ...
    setTimeout(function(){wi=(wi+1)%words.length;el.textContent=words[wi];el.style.opacity='1';el.style.transform='translateY(0)'},350);
  },2800);
}
function initScrollIndicator(){
  ...
  window.addEventListener('scroll',function(){
    ...
    if((sy>100)!==(lastY>100))gsap.to('#scrollIndicator',{opacity:h?0:1,y:h?12:0,duration:.45,ease:'power2.out'});
    lastY=sy;
  },{passive:true});
}
function initReveal(){
  document.querySelectorAll('.event-card,.social-card,.stat-item').forEach(function(el,i){
    ...
    new IntersectionObserver(function(entries){entries.forEach(function(en){if(en.isIntersecting)gsap.to(el,{opacity:1,y:0,duration:.65,delay:(i%4)*.08,ease:'power2.out'})})},{threshold:.15}).observe(el);
  });
  document.querySelectorAll('.section-title,.section-label,.section-desc').forEach(function(el,i){
    gsap.set(el,{opacity:0,y:18});
    new IntersectionObserver(function(entries){entries.forEach(function(en){if(en.isIntersecting)gsap.to(el,{opacity:1,y:0,duration:.75,delay:i*.04,ease:'power3.out'})})},{threshold:.25}).observe(el);
  });
  document.querySelectorAll('.reveal-line').forEach(function(el){
    new IntersectionObserver(function(entries){entries.forEach(function(en){if(en.isIntersecting)el.classList.add('visible')})},{threshold:.4}).observe(el);
  });
  var bw=document.querySelector('.big-cta-wrap');
  if(bw)new IntersectionObserver(function(entries){entries.forEach(function(en){gsap.to('.big-cta-item',{color:en.isIntersecting?'rgba(255,215,0,.22)':'rgba(255,215,0,.07)',duration:.8,ease:'power2.out'})})},{threshold:.2}).observe(bw);
}
fix: heroL1 white-space nowrap - prevent line break in 3D split chars  var steps=document.querySelectorAll('.step');
  steps.forEach(function(s){
    new IntersectionObserver(function(entries){entries.forEach(function(en){if(en.isIntersecting){steps.forEach(function(x){x.classList.remove('active')});en.target.classList.add('active')}})},{threshold:.55,rootMargin:'-15% 0px -15% 0px'}).observe(s);
  });
}
function initAurora(){
...
document.addEventListener('DOMContentLoaded',function(){
  initCursor();initWordSwap();initSwipers();
